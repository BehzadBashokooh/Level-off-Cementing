...
class AppWindow(qtw.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.L = 50 #you can modify the casing and openhole ID size with this (bigger L => bigger ID)
    ...
    def calculate(self):
        ...
        self.rtts_to_shoe_vol = (self.shoe_md - self.rtts_md)*3.281*self.csg_cap
        ...
        if self.final_cmt_col <  self.loss_zone_md - self.shoe_md:
            ...
            self.disp_mud_vol = self.total_dp_vol + self.rtts_to_shoe_vol + (self.oh_vol - self.final_cmt_vol - self.cmt_lost_vol)
            ...
            self.final_cmt_vol = self.oh_vol + (self.final_cmt_col - (self.loss_zone_md - self.shoe_md))*3.281*self.csg_cap
        ...
        self.disp_mud_vol = self.total_dp_vol + self.rtts_to_shoe_vol + self.oh_vol - self.total_cmt_vol
        ...
        self.loss_zone_press_diff = 0.052*self.initial_cmt_col*3.281*self.cmt_weight/7.481 + \
                               0.052*(self.loss_zone_md - self.initial_cmt_col)*3.281*self.mw/7.481 - self.loss_zone_press
    ...
    def populate_output(self):
        ...
        self.ui.loss_zone_press_diff.setText('{:.2f}'.format(self.loss_zone_press_diff))
    ...
    def well_schematic(self):
        ...
        #drillpipe
        dp = go.Scatter(x = [-L,-L,L,L], y=[rtts_md,0,0,rtts_md], line = dict(color = 'black'), mode="lines", fill="toself", fillcolor = 'rgb(160, 162, 163)')
        #casing
        csg = go.Scatter(x = [-4*L,-4*L,4*L,4*L], y=[shoe_md,0,0,shoe_md], line = dict(color = 'black'), mode="lines", fill="toself", fillcolor = 'rgb(113, 121, 126)')
        #casing shoe
        ...
        ohx = np.concatenate((-ohx1,ohx2,np.flip(ohx1)), axis=None) #combine the lines
        ohy = np.concatenate((ohy1,ohy2,np.flip(ohy1)), axis =None)
        self.ohdf = pd.DataFrame({'x': ohx, 'y': ohy})

        ohb = go.Scatter(x = ohx, y = ohy, line = dict(color = 'black'), mode="lines", fill="toself") #bottom
        #RTTS packer
    ...
    def fluids_init(self):
        ...
        if init_toc_md >= shoe_md:
            ...
            cmt = go.Scatter(x = cmtdf['x'], y = cmtdf['y'], line = dict(color = 'rgba(0,0,0,0)'), mode="lines", fill="toself", fillcolor = 'rgb(202, 204, 206)')
            mud = go.Scatter(x = muddf['x'], y = muddf['y'], line = dict(color = 'rgba(0,0,0,0)'), mode="lines", fill="toself", fillcolor = '#FFD700')
            dp_mud = go.Scatter(x = [-L,-L,L,L], y=[rtts_md,0,0,rtts_md], line = dict(color = 'black'), mode="lines", fill="toself", fillcolor = '#FFD700')
            ...
            cmtdf = cmtdf.append({'x': 4*L , 'y':init_toc_md}, ignore_index=True)
            cmt = go.Scatter(x = cmtdf['x'], y = cmtdf['y'], line = dict(color = 'rgba(0,0,0,0)'), mode="lines", fill="toself", fillcolor = 'rgb(202, 204, 206)')
            mud = go.Scatter(x = [-4*L,-4*L,4*L,4*L], y = [init_toc_md,rtts_md,rtts_md,init_toc_md], line = dict(color = 'rgba(0,0,0,0)'), mode="lines", fill="toself", fillcolor = '#FFD700')
            dp_mud = go.Scatter(x = [-L,-L,L,L], y=[rtts_md,0,0,rtts_md], line = dict(color = 'black'), mode="lines", fill="toself", fillcolor = '#FFD700')
    ...
    def fluids_final(self):
        ...
        if final_toc_md >= shoe_md:
            ...
            cmt = go.Scatter(x = cmtdf['x'], y = cmtdf['y'], line = dict(color = 'rgba(0,0,0,0)'), mode="lines", fill="toself", fillcolor = 'rgb(202, 204, 206)')
            mud = go.Scatter(x = muddf['x'], y = muddf['y'], line = dict(color = 'rgba(0,0,0,0)'), mode="lines", fill="toself", fillcolor = '#FFD700')
            dp_mud = go.Scatter(x = [-L,-L,L,L], y=[rtts_md,mud_drop_dp,mud_drop_dp,rtts_md], line = dict(color = 'black'), mode="lines", fill="toself", fillcolor = '#FFD700')
            ...
            cmtdf.index = cmtdf.index+1
            cmtdf = cmtdf.sort_index()
            cmtdf = cmtdf.append({'x': 4*L , 'y':final_toc_md}, ignore_index=True)
            cmt = go.Scatter(x = cmtdf['x'], y = cmtdf['y'], line = dict(color = 'rgba(0,0,0,0)'), mode="lines", fill="toself", fillcolor = 'rgb(202, 204, 206)')
            mud = go.Scatter(x = [-4*L,-4*L,4*L,4*L], y = [final_toc_md,rtts_md,rtts_md,final_toc_md], line = dict(color = 'rgba(0,0,0,0)'), mode="lines", fill="toself", fillcolor = '#FFD700')
            dp_mud = go.Scatter(x = [-L,-L,L,L], y=[rtts_md,0,0,rtts_md], line = dict(color = 'black'), mode="lines", fill="toself", fillcolor = '#FFD700')
    ...
    def show_fig(self, chart_type):
        ...
        if chart_type == 'initial':
            self.fig_data = self.well_schematic_data + self.fluids_init_data + self.csg_shoe_icons #[csg,dp,ohb,rtts_r,rtts_l,lzr,lzl,cmt, mud, dp_mud,csgr_tri,csgl_tri]
        elif chart_type == 'final':
            self.fig_data = self.well_schematic_data + self.fluids_final_data + self.csg_shoe_icons #[csg,dp,ohb,rtts_r,rtts_l,lzr,lzl,cmt, mud, dp_mud,csgr_tri,csgl_tri]
